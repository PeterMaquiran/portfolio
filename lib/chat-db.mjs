...
export function getDb() {
  ...
  if (!names.has('device')) db.exec("ALTER TABLE conversations ADD COLUMN device TEXT NOT NULL DEFAULT ''")
  if (!names.has('os')) db.exec("ALTER TABLE conversations ADD COLUMN os TEXT NOT NULL DEFAULT ''")

  const tokenColumns = db.prepare('PRAGMA table_info(fcm_tokens)').all()
  const tokenNames = new Set(tokenColumns.map((column) => column.name))
  if (!tokenNames.has('role')) db.exec("ALTER TABLE fcm_tokens ADD COLUMN role TEXT NOT NULL DEFAULT 'visitor'")
...
export function upsertConversation({ id, ip, country, countryCode, device, os }) {
  const now = Date.now()
  getDb()
    .prepare(
      `
      INSERT INTO conversations (id, ip, country, country_code, created_at, device, os)
      VALUES (@id, @ip, @country, @countryCode, @now, @device, @os)
      ON CONFLICT(id) DO UPDATE SET
        ip = excluded.ip,
        country = excluded.country,
        country_code = excluded.country_code,
        device = COALESCE(NULLIF(excluded.device, ''), conversations.device),
        os = COALESCE(NULLIF(excluded.os, ''), conversations.os)
    `,
    )
    .run({
      id,
...
export function ackMessages({ ids, recipient, conversationId }) {
  if (!ids?.length) return { deleted: 0, conversationIds: [] }
  const placeholders = ids.map(() => '?').join(',')
  const lookup =
    recipient === 'admin'
      ? getDb().prepare(
          `SELECT conversation_id AS conversationId FROM messages WHERE sender = 'visitor' AND id IN (${placeholders})`,
        )
      : getDb().prepare(
          `SELECT conversation_id AS conversationId FROM messages WHERE sender = 'admin' AND conversation_id = ? AND id IN (${placeholders})`,
        )
  const rows = recipient === 'admin' ? lookup.all(...ids) : lookup.all(conversationId, ...ids)
  ...
  return { deleted, conversationIds }
}
...
export function saveFcmToken(token, visitorId, role = 'visitor') {
  const now = Date.now()
  const id = typeof visitorId === 'string' && visitorId ? visitorId : null
  const nextRole = role === 'admin' ? 'admin' : 'visitor'
  getDb()
    .prepare(
      `
      INSERT INTO fcm_tokens (token, visitor_id, role, created_at, last_seen)
      VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(token) DO UPDATE SET
        visitor_id = COALESCE(excluded.visitor_id, fcm_tokens.visitor_id),
        role = excluded.role,
        last_seen = excluded.last_seen
    `,
    )
    .run(token, id, nextRole, now, now)
}

export function listVisitorTokens(visitorId) {
  if (!visitorId) return []
  return getDb()
    .prepare(
      `SELECT token FROM fcm_tokens WHERE visitor_id = ? AND role = 'visitor'`,
    )
    .all(visitorId)
    .map((row) => row.token)
}
