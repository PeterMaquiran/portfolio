...
import { notifyAdminsOfVisitorMessage, notifyVisitorIfInactive } from './chat-notify.mjs'
...
export function attachChat(io) {
  ...
  io.on('connection', (socket) => {
    socket.on('chat:hello', async (payload, ack) => {
      ...
      socket.data.pageActive = payload?.pageActive !== false
    ...
    socket.on('chat:presence', (payload) => {
      socket.data.pageActive = Boolean(payload?.active)
    })
    ...
    socket.on('chat:send', (payload, ack) => {
      ...
      if (!duplicate) {
        ...
        if (sender === 'visitor') {
          void notifyAdminsOfVisitorMessage(conversation, text).catch((error) => {
            console.error('Admin chat notification failed:', error)
          })
        } else {
          void notifyVisitorIfInactive(io, conversationId, text).catch((error) => {
            console.error('Visitor chat notification failed:', error)
          })
        }
      }
